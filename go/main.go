...
func getTrend(c echo.Context) error {
	...
	for _, character := range characterList {
		type IsuWithTimestamp struct {
			Isu
			Timestamp time.Time `db:"timestamp"`
			Condition string    `db:"condition"`
		}
		isuList := []IsuWithTimestamp{}
		err = db.Select(&isuList,
			"SELECT `isu`.*, `timestamp`, `condition` FROM `isu` WHERE `character` = ? JOIN `isu_condition` ON  `isu_condition`.`jia_isu_uuid` = `isu`.`jia_isu_uuid`",
			character.Character,
		)
		if err != nil {
			c.Logger().Errorf("db error: %v", err)
