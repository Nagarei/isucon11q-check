...
func getIsuIcon(c echo.Context) error {
	...
	err = db.Get(&image_hash, "SELECT `image_hash` FROM `isu` WHERE `jia_user_id` = ? AND `jia_isu_uuid` = ?",
		jiaUserID, jiaIsuUUID)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return c.String(http.StatusNotFound, "not found: isu")
...
retry_loop:
	for retry {
		...
		rows, err := db.Queryx("SELECT * FROM `isu_condition` WHERE `jia_isu_uuid` = ? AND ? <= `timestamp` AND `timestamp` < ? ORDER BY `timestamp` ASC",
...
func getIsuConditions(c echo.Context) error {
	...
	for _, level := range strings.Split(conditionLevelCSV, ",") {
		if level == "info" {
			conditionLevel = append(conditionLevel, "i")
		} else if level == "warning" {
			conditionLevel = append(conditionLevel, "w")
		} else if level == "critical" {
			conditionLevel = append(conditionLevel, "c")
		}
	}
...
func getIsuConditionsFromDB(db *sqlx.DB, jiaIsuUUID string, endTime time.Time, conditionLevel []string, startTime time.Time,
	...
	for _, c := range conditions {
		cLevel := "warning"
		if c.ConditionLevel == "c" {
			cLevel = "critical"
		} else if c.ConditionLevel == "i" {
			cLevel = "info"
		}
...
var trendCacheMutex sync.RWMutex
var trendCache []byte
...
loopstart:
	for {
		...
		for _, cond := range conditions {
			if _, ok := m[cond.Character]; !ok {
				m[cond.Character] = &TrendResponse{
					Character: cond.Character,
					Info:      make([]*TrendCondition, 0, 100),
					Warning:   make([]*TrendCondition, 0, 100),
		...
		for _, cond := range m {
			sort.Slice(cond.Info, func(i, j int) bool {
				return cond.Info[i].Timestamp > cond.Info[j].Timestamp
			})
			sort.Slice(cond.Warning, func(i, j int) bool {
				return cond.Warning[i].Timestamp > cond.Warning[j].Timestamp
			})
			sort.Slice(cond.Critical, func(i, j int) bool {
				return cond.Critical[i].Timestamp > cond.Critical[j].Timestamp
			})
			res = append(res, TrendResponse{Character: cond.Character, Info: cond.Info, Warning: cond.Warning, Critical: cond.Critical})
		}
